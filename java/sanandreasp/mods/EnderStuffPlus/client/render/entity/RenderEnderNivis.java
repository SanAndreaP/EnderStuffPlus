...
public class RenderEnderNivis extends RenderLiving implements Textures {
	...
	private ModelEnderNivis endermanModel;
	private Random rnd;

	public RenderEnderNivis(ModelEnderNivis model, float f) {
		super(model, f);
		this.rnd = new Random();
		this.endermanModel = (ModelEnderNivis) super.mainModel;
		this.setRenderPassModel(this.endermanModel);
	}

	public void renderEnderman(EntityEnderNivis entityenderman, double d,
	...
	protected void renderCarrying(EntityEnderNivis entityenderman, float f) {
	...
	protected int renderEyes(EntityEnderNivis entityenderman, int i,
			float f) {
		if( i != 0 ) {
	...
	protected int shouldRenderPass(EntityLivingBase entityliving, int i, float f) {
		return this.renderEyes((EntityEnderNivis) entityliving, i, f);
	}

	protected void renderEquipped(EntityEnderNivis entityliving, float f) {
		super.renderEquippedItems(entityliving, f);
		ItemStack itemstack = entityliving.getHeldItem();
		if( itemstack != null ) {
			GL11.glPushMatrix();
			this.endermanModel.bipedRightArm2.postRender(0.0625F);
	...
	protected void renderEquippedItems(EntityLivingBase entityliving, float f) {
		this.renderCarrying((EntityEnderNivis) entityliving, f);
		this.renderEquipped((EntityEnderNivis) entityliving, f);
	}

	@Override
	public void doRenderLiving(EntityLiving entityliving, double d, double d1,
			double d2, float f, float f1) {
		this.renderEnderman((EntityEnderNivis) entityliving, d, d1, d2, f, f1);
	}

	@Override
	public void doRender(Entity entity, double d, double d1, double d2,
			float f, float f1) {
		this.renderEnderman((EntityEnderNivis) entity, d, d1, d2, f, f1);
	}
