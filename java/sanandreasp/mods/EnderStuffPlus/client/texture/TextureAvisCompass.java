    ...
    extends TextureAtlasSprite
{
    ...
    public void updateAnimation() {
        ...
        if( mc.theWorld != null && mc.thePlayer != null ) {
            if( ItemAvisCompass.getNearestEgg(MathHelper.floor_double(mc.thePlayer.posX),
                                               MathHelper.floor_double(mc.thePlayer.posZ)) != null )
            {
    ...
    public void updateCompass(World world, double playerX, double playerZ, double playerYaw, boolean notInWorld) {
        ...
        for( newFrameCnt = (int) ((this.currentAngle / (Math.PI * 2D) + 1.0D) * this.framesTextureData.size())
                           % this.framesTextureData.size(); newFrameCnt < 0; newFrameCnt = (newFrameCnt + this.framesTextureData.size())
        ...
        if( newFrameCnt != this.frameCounter ) {
            ...
            TextureUtil.uploadTextureSub((int[]) this.framesTextureData.get(this.frameCounter), this.width,
