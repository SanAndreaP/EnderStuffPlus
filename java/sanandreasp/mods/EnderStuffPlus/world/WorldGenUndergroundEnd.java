    ...
    extends WorldGenerator
{
                ...
                for( int j1 = radius - randomFactor / 2 + random.nextInt(randomFactor + 1); j1 > minimum; j1-- ) {
                    ...
                    if( Math.pow(i1, 2) + Math.pow(j1, 2) + Math.pow(k1, 2) < Math.pow(radius, 2) ) {
                        ...
                        if( this.canEdit(world, i + i1, j + j1, k + k1) ) {
                            ...
                            // (right now it's lava)
                            if( j + j1 < 10 ) {
                                world.setBlock(i + i1, j + j1, k + k1, Block.waterStill.blockID, 0,
                                               random.nextBoolean() ? 2 : 3);
                            }
                            // Hollowing area and filling in the bottom middle
                            // (minimum is used here)
                            else if( j1 == minimum + 2 && world.getBlockId(i + i1, j + j1 + 1, k + k1) == 0
                                     && world.getBlockId(i + i1, j + j1, k + k1) != 0 ) {
                                if( random.nextInt(42) == 0 ) {
                                    this.plantObsidianPillar(world, random, i + i1, j + j1 + 1, k + k1,
                                                             radius - random.nextInt(5));
                                }

                                world.setBlock(i + i1, j + j1, k + k1, Block.whiteStone.blockID, 0,
                                               random.nextBoolean() ? 2 : 3);
                                world.setBlock(i + i1, j + j1 - 1, k + k1, Block.whiteStone.blockID, 0,
                                               random.nextBoolean() ? 2 : 3);
                                ...
                                if( j1 != minimum + 1 ) {
                                    boolean b = false;
                                    for( int y = 1; y < radius
                                                    && world.getBlockId(i + i1, j + j1 + y - 1, k + k1) != Block.obsidian.blockID; y++ ) {
                                    ...
                                    if( b ) {
                                        world.setBlock(i + i1, j + j1, k + k1, Block.obsidian.blockID, 0,
                                                       random.nextBoolean() ? 2 : 3);
                                    } else if( world.getBlockId(i + i1, j + j1 + 1, k + k1) != Block.waterStill.blockID
                                               && world.getBlockId(i + i1, j + j1 + 1, k + k1) != Block.waterMoving.blockID ) {
                    ...
                    // Putting mushroom grass and dirt etc. onto the side areas
                        ...
                        if( this.canEdit(world, i + i1, j + j1, k + k1) ) {
                            if( world.getBlockId(i + i1, j + j1 + 1, k + k1) == 0
                                && world.getBlockId(i + i1, j + j1, k + k1) != 0 && j1 < 6 ) {
                                if( j + j1 < 10 ) {
                                    world.setBlock(i + i1, j + j1, k + k1, Block.waterStill.blockID, 0,
                                                   random.nextBoolean() ? 2 : 3);
                                } else {
    ...
    public boolean recursiveGenerateSpheres(World world, Random random, int i, int j, int k, int initialRadius) {
        ...
        for( int c = 0; c < 3 + random.nextInt(3); c++ ) {
            ...
            int k1 = 0;
            while( Math.abs(i1) + Math.abs(k1) < radius * 3 / 2
                   && world.getBlockId(i + i1, j - j1, k + k1) != Block.stone.blockID ) {
