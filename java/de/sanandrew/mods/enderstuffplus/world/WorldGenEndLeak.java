        ...
        for( int i = -radius; i <= radius; i++ ) {
            for( int k = -radius; k <= radius; k++ ) {
                for( int j = -radius - 1; j < 0; j++ ) {
                    ...
                    Block baseBlock = random.nextInt(10) == 0 ? Blocks.end_stone : ModBlockRegistry.corruptES;

                    if( radVec <= rad6 ) {
                        if( !this.replace(world, x + i, y + j, z + k, Blocks.air) ) {
                            if( this.isInEnd && world.getBlock(x + i, y + j, z + k) == ModBlockRegistry.corruptES ) {
                        ...
                        if( random.nextInt(5) == 0 ) {
                            ...
                            if( !this.replace(world, x + i, y + j, z + k, Blocks.air)
                                ...
                                if( this.isInEnd
                                    && world.getBlock(x + i, y + j, z + k) == ModBlockRegistry.corruptES ) {
                                    // world.setBlockToAir(x+i, y+j, z+k);;
                                } else {
