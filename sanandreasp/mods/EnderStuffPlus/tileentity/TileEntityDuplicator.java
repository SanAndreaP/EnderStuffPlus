...
public class TileEntityDuplicator extends TileEntity implements IInventory {
	...
	private int getStackSize(ItemStack is) {
		if( is == null ) return 0;
		return is.stackSize;
	}
		...
		if( !worldObj.isRemote ) {
			...
			if( active ) {
				if( this.burnTime > 0 ) {
					...
					this.dupeInv[2] = CUS.decrStackSize(this.dupeInv[2]);
				}
				if( this.procTime < 20 ) {
					...
					this.dupeInv[0] = CUS.decrStackSize(this.dupeInv[0]);
					this.dupeInv[1] = CUS.decrStackSize(this.dupeInv[1]);
