...
//		}
		...
		for( int i = 0; i < rad*2; i++ ) {
			...
			generateTable(world, random, x+x1, y+y1, z+z1, 8+random.nextInt(8));
		}
		...
		if( random.nextInt(32) == 0 ) {
			...
			for( int i = 0; i < 4; i++ ) {
				(new WorldGenLakes(Block.glowStone.blockID)).generate(world, random, x+random.nextInt(8)-4, world.getHeightValue(x, z), z+random.nextInt(8)-4);
			}
			for( int i = 0; i < 16; i++ ) {
			...
			for( int k = -rad; k < rad; k++ ) {
				for( int j = -rad; j <= 0; j++ ) {
					if( i*i+k*k > (rad-Math.abs(j))*(rad-Math.abs(j)) )
						...
						createSpike(world, random, x+i, y, z+k, rad-Math.round((float)Math.sqrt(i*i+k*k)));
					}
						
					setBlock(world, x+i, y+j, z+k, Block.whiteStone.blockID);
				}
			}
		}
	}
