...
public class GuiBiomeChangerFuel extends GuiBiomeChangerBase implements Textures {
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		String s = ESPModRegistry.manHelper.getLangMan().getTranslated("enderstuffplus.biomeChanger.gui1.range") + " " + ((Container_BiomeChanger)this.inventorySlots).biomeChanger.getMaxRange();
        ...
        for( int i = this.entryPos; i < 4 + this.entryPos && i < this.fuelEntries.size(); i++ ) {
        	...
        	if( inavailable ) {
                this.drawRect(x-1, y-1, x+152, y+17, 0x60000000);
        	}
    		RenderHelper.disableStandardItemLighting();
